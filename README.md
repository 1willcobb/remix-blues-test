...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create RemixBlues-84cb
  fly apps create RemixBlues-84cb-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app RemixBlues-84cb
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app RemixBlues-84cb-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'RemixBlues-84cb-staging' does not match app name in config file 'RemixBlues-84cb'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name RemixBlues-84cb-db
  fly postgres attach --app RemixBlues-84cb RemixBlues-84cb-db

  fly postgres create --name RemixBlues-84cb-staging-db
  fly postgres attach --app RemixBlues-84cb-staging RemixBlues-84cb-staging-db
  ```
